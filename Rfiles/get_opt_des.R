                        ...
                        lambda_H1, H1_type, HR1, HR2, change_time,tau = NULL, alpha, power = NULL) 
{
    ...
    r <- N / acc_time
    int_factor <- seq(0.38, 0.7, by = int_step / N)  # Each time interim sample size increase by 6
    ...
    {  
        ...
        if (method != 'logrank')
        {
            ...
            if (method == 'Complex'){
                best_our <- best_our[which.min(fil_combs$power), ]
            }
        }
    ...
    if (dim(all_result)[1] == 0) {
        if(method == 'logrank'){
            return(data.frame(m1 = 0, m2 = 0, PET0 = 0, PET1 = 0, alpha = 0, 
             ...
             return(data.frame(m1 = 0, q1 = 0, m2 = 0, q2 = 0, lambda = 0, gamma = 0, 
